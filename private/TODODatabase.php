...
class TODODatabase {
    ...
    public function insert_autogenerated_projects(): void {
        $user_id = SQLite3::escapeString($this->user_id);
        $res = $this->db->exec("INSERT INTO Project(UserId, ProjectName, IsAutoGenerated) VALUES
                    ($user_id, 'Inbox', 1), ($user_id, 'Today', 1), ($user_id, 'Upcoming', 1),
                    ($user_id, 'Completed', 1), ($user_id, 'Deleted', 1)");
        if (!$res) {
